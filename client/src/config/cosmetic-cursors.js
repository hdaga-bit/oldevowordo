/** Client cursor trail tokens — must stay in sync with server. */
...
export function getCursorListForPicker(unlockedIds = []) {
  const set = new Set(unlockedIds);
  return Object.values(COSMETIC_CURSORS).map((c) => ({
    ...c,
    unlocked:
      c.id === DEFAULT_CURSOR_ID ||
      set.has(`cursor:${c.id}`) ||
      set.has(c.id),
  }));
}
